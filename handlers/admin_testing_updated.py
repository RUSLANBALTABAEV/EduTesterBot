# handlers/admin_testing_updated.py
"""
Обработчики для администрирования тестирования с поддержкой Excel.
"""
import io
import json
...
async def list_all_tests(callback: types.CallbackQuery):
    ...
    async with async_session() as session:
        # Получаем все тесты с информацией о курсах
...
async def confirm_test_creation(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        ...
        f"Теперь добавьте вопросы через меню 'Добавить вопросы'"
    )
    await state.clear()
    await callback.answer()


@admin_testing_router.callback_query(F.data == "cancel_test")
...
async def show_test_statistics(callback: types.CallbackQuery):
    ...
    async with async_session() as session:
        ...
        text = (
            f"📊 <b>Статистика теста: {test.title}</b>\n\n"
